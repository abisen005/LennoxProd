trigger MaterialDeleteTrigger on SCSkus__c (before delete) {

bypasstrigger__c settings=bypasstrigger__c.getInstance('forecast');
...
if(bypass_triggers !=true){
    User u = new User();
    for(SCSkus__c record: Trigger.new==null? Trigger.old: Trigger.new) {
        ...
        record.addError('You do not have permission to delete a record.');
    }
}}
